...
export default class RoleRepo implements IRoleRepo {
  ...
  public async listAll(): Promise<Role[]> {
    return new Promise<Role[]>((resolve, reject) => {
      this.roleSchema.find({}, (error: any, result: Role[]) => {
        if (error) reject(error);
        else {
          let role: Role[] = [];
          result.forEach(function (element: Role) {
            role.push(element);
          });
          resolve(role);
        }
  ...
  public async findByName(name: string): Promise<Role> {
    const query = { name: name }; // Search query by name
    ...
    try {
      ...
      if (roleRecord) {
        ...
        return null; // Role with the given name not found
      }
    } catch (error) {
      throw error;
    }
