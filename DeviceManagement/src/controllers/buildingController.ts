...
export default class BuildingController implements IBuildingController /* TODO: extends ../core/infra/BaseController */ {
  ...
  public async createBuilding(req: Request, res: Response, next: NextFunction) {
    try {
      console.log("Chegou ao controller createBuilding")
  ...
  public async listBuildings(req: Request, res: Response, next: NextFunction) {
    try {
			const buildingOrError = (await this.buildingServiceInstance.listBuildings()) as Result<IBuildingDTO[]>;

			if (buildingOrError.isFailure) {
				return res.status(402).json(buildingOrError.error);
			}
