...
caminho_pisosInternos(CelAct, PiAct, CelDest, PiDest, [], [Cam], FinalWeight):-
    ...
    write('Debug: FinalWeight = '), write(FinalWeight), nl,
    !.
...
caminho_pisosInternos(CelAct, PiAct, CelDest, PiDest, [cor(PisoOr, PisoDestInt)|RestLigs], [Cam|RestoCams], TotalWeight):-
    passagem(_, _, PisoOr, PiDestInt, CelOr, CelDestInt),
    write('Debug: passagem - PisoOr = '), write(PisoOr), write(', PiDestInt = '), write(PiDestInt), write(', CelOr = '), write(CelOr), write(', CelDestInt = '), write(CelDestInt), nl,

    %Calcular caminho entre célula atual e a célula do inicio da ligacao
    aStarPath(CelAct, CelOr, PisoDestInt, Cam, Weight),
    write('Debug: aStarPath - Cam = '), write(Cam), write(', Custo = '), write(Custo), nl,
    ...
    write('Debug: caminho_pisosInternos - CalcWeight = '), write(CalcWeight), nl,

    TotalWeight is Weight + CalcWeight,
    ground(TotalWeight),  % Ensure TotalWeight is ground (instantiated)

    write('Debug: TotalWeight = '), write(TotalWeight), nl,
    !.
...
caminho_pisosInternos(CelAct, PiAct, CelDest, PiDest, [elev(PisoOr, PisoDest)|RestLigs], [Cam|RestoCams], TotalWeight):-
    ...
    member(PiDest, LElevPisos),
    write('Debug: elevador - PisoOr = '), write(PisoOr), write(', PisoDest = '), write(PisoDest), write(', CelElev = '), write(CelElev), nl,
