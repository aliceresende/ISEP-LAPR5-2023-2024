...
var tween;
var keepGoing = true;
/*var auto_path = [[0,5,"Andar 1 Edifício C"],[1,5,"Andar 1 Edifício C"],[2,5,"Andar 1 Edifício C"],[3,5,"Andar 1 Edifício C"],
                    [4,5,"Andar 1 Edifício C"],[6,5,"Andar 1 Edifício C"],[7,5,"Andar 1 Edifício C"],[8,5,"Andar 1 Edifício C"],
                    [9,5,"Andar 1 Edifício C"],[10,5,"Andar 1 Edifício C"],
 ]*/
 var auto_path = [[0,5,"Andar 1 Edifício C"],[1,5,"Andar 1 Edifício C"],[2,5,"Andar 1 Edifício C"],[3,5,"Andar 1 Edifício C"],
                    [4,5,"Andar 1 Edifício C"],[5,5,"Andar 1 Edifício C"],[5,6,"Andar 1 Edifício C"],[5,7,"Andar 1 Edifício C"],
                    [5,8,"Andar 1 Edifício C"],[5,9,"Andar 1 Edifício C"],[5,10,"Andar 1 Edifício C"],
                    [0,5,"Andar 2 Edificio C"],[0,5,"Andar 2 Edifício C"],[1,5,"Andar 2 Edifício C"],[2,5,"Andar 2 Edifício C"],[3,5,"Andar 2 Edifício C"],
                    [4,5,"Andar 2 Edifício C"],[5,5,"Andar 2 Edifício C"],[6,5,"Andar 2 Edifício C"],[7,5,"Andar 2 Edifício C"],
                    [8,5,"Andar 2 Edifício C"],[9,5,"Andar 2 Edifício C"],[10,5,"Andar 2 Edifício C"]               
 ]
...
export default class ThumbRaiser {
    ...
    constructor(generalParameters, mazeParameters, playerParameters, lightsParameters, fogParameters, fixedViewCameraParameters, firstPersonViewCameraParameters, thirdPersonViewCameraParameters, topViewCameraParameters, miniMapCameraParameters) {
       ...
       // this.automaticMovementTable= document.getElementById("automaticMovement");
        ...
        window.addEventListener('keydown', event => this.maze.keyChange(event, true));

        window.addEventListener('keyup', event => this.maze.keyChange(event, false));
   ...
   setAutomaticMovementTableVisibility(visible){
    ...
    keyChange(event, state) {
        ...
        if (document.activeElement == document.body) {
            ...
            /* To-do #41 - Toggle the user interface visibility
                ...
                - state: true*/
            if (this.player.keyStates.userInterface && this.player.state) { // Display / hide user interface
    ...
    elevatorCollision(position){
        return this.maze.distanceToWestElevator(position) < this.player.radius ||
               this.maze.distanceToNorthElevator(position) < this.player.radius;
    }
    passageCollision(position){
        return this.maze.distanceToPassage(position) < this.player.radius
        
    }
    doorCollision(position){
    ...
    update() {
        ...
        if (!this.gameRunning) {
            ...
            this.route.remaining_cels = auto_path;
        }
        else {
            let playerMoved = false;
            //console.log(this.maze);
           
            const floor_number = this.maze.floorCode;
            ...
            if (!this.animations.actionInProgress) {
                ...
                if (this.maze.foundExit(this.player.position)) {
                    ...
                    /* To-do #12 - Compute the distance covered by the player
                        ...
                        - elapsed time: deltaT*/
                    coveredDistance = this.player.walkingSpeed * deltaT;
                    ...
                    */
                    ...
                    if (this.player.keyStates.backward) { // The player is moving backward
                        ...
                        /*if (this.doorCollision(newPosition)) {
                            console.log(this.maze);
                        }
                        else {
                            this.animations.fadeToAction(this.player.keyStates.run ?  "Running" : "Walking",  0.2);
                    ...
                    function redirectToNewPage(page_html) {
                        // Replace 'new-page.html' with the path to your HTML file
                        window.location.href = page_html;
                    }
                            ...
                            fetch(`http://localhost:3000/api/floors/listFloors/`)
                                ...
                                .then(floorData => {
                                    ...
                                    floorData.forEach(floor => {
                                        console.log(floor);
                                        if (floor.id === floorID) {
                                            const building = floor.floorBuilding;
                                            resolve(building); // Resolve the promise with the map data
                                        }
                                    });
                                })
                                .catch(error => {
                    ...
                    if (this.passageCollision(newPosition)) {
                        console.log("passagem");
                        console.log(this.maze);
                        redirectToNewPage(`http://192.168.1.70:5500/3DView/SGRAI/Thumb_Raiser_auto.html?floor=${this.maze.passageCode}`);
                    }
                    else {
                        this.animations.fadeToAction(this.player.keyStates.run ?  "Running" : "Walking",  0.2);
                        this.player.position = newPosition;
                    ...
                    if (this.elevatorCollision(newPosition)) {
                        const queryString = window.location.search;
                        console.log(queryString);
                        const urlParams = new URLSearchParams(queryString);
                        const floorID = urlParams.get('floor');
                        console.log("floor para ir",this.route.currentFloor);
                        const building=fetchDataFromDatabase(floorID);
                       building.then((result) => {
                           redirectToNewPage(`elevator_gui_auto.html?building=${result}&floorCode=${this.route.currentFloor}`);
                        });
                    }
                    else {
                        this.animations.fadeToAction(this.player.keyStates.run ?  "Running" : "Walking",  0.2);
            ...
            if(keepGoing){
                ...
                while(this.route.remaining_cels.length > 0){
                    if(this.route.remaining_cels[0][2] == floor_number){
                        keepGoing = false;
                        console.log(this.route.remaining_cels[0][2]);
                        console.log(floor_number);
                        this.player.keyStates.forward=true;
                        const fromCoords = this.player.position;
                        
                        const toCoords = this.maze.cellToCartesian(this.route.remaining_cels.shift());
                        if (auto_path[0][2]!== this.route.currentFloor) {
                            console.log("Floor changed from", this.route.currentFloor, "to", auto_path[0][2]);
                            this.route.currentFloor = auto_path[0][2];
                        }
                        
                        tween = new TWEEN.Tween(fromCoords)
                            .to({x: toCoords.x, y: toCoords.y, z: toCoords.z}, 1000)
                            .easing(TWEEN.Easing.Linear.None)
                            .onUpdate(() => {
                                playerMoved = true;
                                this.player.position.set(fromCoords.x, fromCoords.y, fromCoords.z);
                            })
                            .start()
                            .onComplete(() => {
                                keepGoing = true;
                        ...
                        this.route.remaining_cels.shift();
                    }
                    
                }
                 // If no matching cell is found, clear the entire matrix
                if (this.route.remaining_cels.length === 0) {
                    this.route.remaining_cels = [];
                }
            }
